...
export class UsersService {
  ...
  findOneDeletedFalseDisabledFalse(
    userId: number
  ...
  findOneByUsernameWithPassword(username: string): Observable<UserDto> {
    return from(
      this.userModel
        .findOne({ userName: username, deleted: false, disabled: false })
        .exec()
        .then((user) => {
          if (user) {
            const userDoc = { ...user['_doc'] };
  ...
  create(createUserDto: CreateUserDto): Observable<ResponseUserDto> {
  ...
  registerUser(registerUsersDto: RegisterUserDto): Observable<string> {
